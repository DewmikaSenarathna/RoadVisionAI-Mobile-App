...
ALLOWED_ORIGINS = [origin.strip() for origin in os.getenv("ROADVISIONAI_CORS_ORIGINS", "*").split(",") if origin.strip()]
...
def fetch_weather(latitude: float, longitude: float) -> dict[str, float]:
    # If OpenWeather is configured via environment variables, prefer it
    ow_base = os.getenv("ROADVISIONAI_WEATHER_API_BASE_URL", "").strip()
    ow_key = os.getenv("ROADVISIONAI_WEATHER_API_KEY", "").strip()
    current: dict = {}
    if ow_base and ow_key and "openweathermap" in ow_base:
        try:
            resp = requests.get(f"{ow_base.rstrip('/')}/weather", params={
                "lat": latitude,
                "lon": longitude,
                "appid": ow_key,
                "units": "imperial",
            ...
            # Map OpenWeather response to our internal 'current' shape
            current = {
                "temperature_2m": main.get("temp"),
                "windchill_2m": main.get("temp"),
                "relative_humidity_2m": main.get("humidity"),
                "pressure_msl": main.get("pressure"),
                "visibility": data.get("visibility"),
                "windspeed_10m": wind.get("speed"),
                "precipitation": 0.0,
                ...
                "weather_code": weather_obj.get("id", 0),
            }
        except Exception:
            current = {}
    else:
